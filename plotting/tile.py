...
#
# This just makes wind barbs at the moment
# Quivers haven't been implemented
#
def quivers(projection, x, y, z, args):
    lat, lon = get_latlon_coords(projection, x, y, z)
    if len(lat.shape) == 1:
        lat, lon = np.meshgrid(lat, lon)
    ...
    with open_dataset(config) as dataset:
        ...
        for v in variable:
            all_vars.append(v)
            lon, lat = self.basemap.makegrid(50, 50)
            d = dataset.get_area(
                np.array([lat, lon]),
                depth,
                time,
...
def wind_barbs(projection, x, y, z, args):
    ...
    ax = plt.Axes(fig, [0, 0, 1, 1])
    plt.axis('off')
    plt.barbs(data[0], data[1], U, V, pivot='middle')
