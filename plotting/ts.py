...
class TemperatureSalinityPlotter(PointPlotter):
    def __init__(self, dataset_name: str, query: str, **kwargs):
        ...
        super(TemperatureSalinityPlotter, self).__init__(dataset_name, query, **kwargs)
    ...
    def load_data(self):
        ...
        with open_dataset(
            self.dataset_config,
            timestamp=self.time,
            variable=[temp_var_key, sal_var_key],
        ) as ds:
    ...
    def plot(self):
        ...
        if self.unitSelection:
            ...
            depths, self.data, self.variable_units = utils.convert_to_imperial(self.unitSelection, depths, self.data, self.variables, self.variable_units)
        ...
        tmin = np.amin(self.temperature) - (np.abs(np.amax(self.temperature) * 0.1))
        tmax = np.amax(self.temperature) + (np.abs(np.amax(self.temperature) * 0.1))
