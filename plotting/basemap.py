...
def load_map(
    projection: str, center: tuple, height: float, width: float, min_lat: float = 0
) -> Basemap:
        ...
        try:
            ...
            if projection in ["npstere", "spstere"]:
                basemap = Basemap(
                    ...
                    area_thresh=((height * width) / (1000 * 1000))
                    * 0.00001,  # display islands whose area is 0.001% of displayed area
                    ellps="WGS84",
...
def _get_filename(projection: str, center: tuple, height: float, width: float) -> str:
    hash = hashlib.sha1(
        ";".join(str(x) for x in [projection, center, height, width]).encode()
    ).hexdigest()
    return hash + ".pickle"
