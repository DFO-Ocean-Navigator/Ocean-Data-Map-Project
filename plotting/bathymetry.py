...
class BathPlotter(Plotter):
    ...
    def plot(self):
        ...
        my_plot_div = plot({
            "data": [Surface(z=bathymetry, x=self.longitude, y=self.latitude, colorscale='Earth', colorbar={"len":0.5, "x":-0.85}, showscale=True, cmin=np.nanmin(bathymetry), cmax=0), Surface(z=data, x=self.longitude, y=self.latitude, colorscale='Electric', showscale=True, cmin=np.nanmin(bathymetry), cmax=0)],
            "layout": layout
        }, output_type='div',)
