...
from tests.temp_env_var import ENV_VARS_TO_SUSPEND, TEMP_ENV_VARS
...
# def test_class4_query_endpoint():
#     response = client.get("/api/v1.0/class4")
...
# def test_class4_file_endpoint():
#     response = client.get(
#         "/api/v1.0/class4/ocean_predict?projection=EPSG:3857"
#         "&resolution=9784""&extent=-15936951,1567587,4805001,12398409"
...
# def test_kml_query_endpoint():

#     # response for each type of query
#     response = []
#     response.append(client.get("/api/v1.0/kml/points"))
#     response.append(client.get("/api/v1.0/kml/lines"))
#     response.append(client.get("/api/v1.0/kml/areas"))
#     for i in range(4):
#         assert response[i].status_code == 200


# def test_kml_file_endpoint():
#     response = []
...
#     response = client.get(
#         "/api/v1.0/tiles/giops_real/votemper/2212704000/0/6/50/40?projection=EPSG:3857"
#         "&scale=-5,30&interp=gaussian&radius=25&neighbours=10"
#     )
...
def test_bath_endpoint():
    response = client.get("api/v1.0/tiles/bath/6/56/41?projection=EPSG:3857")

    assert response.status_code == 200


def test_mbt_endpoint():
    response = client.get("/api/v1.0/mbt/lands/7/105/77?projection=EPSG:3857")

    assert response.status_code == 200


@patch("data.observational.queries.get_datatypes")
def test_observation_datatypes(patch_get_datatypes):
