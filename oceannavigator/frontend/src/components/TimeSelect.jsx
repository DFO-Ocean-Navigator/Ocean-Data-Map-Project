...
export default class TimeSelect extends React.Component {
  ...
  /*
    ...
    */
  parseDate(value) {
    const  dateObj = moment.tz(value, "GMT");
    return dateObj;
  }
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        for (const val in response) {
          ...
          const date = this.parseDate(response[val].value);
  ...
  monthsBetween(date1, date2) {
    ...
    const month1 = date1.get("month");
    const month2 = date2.get("month");

    const difference = month2 - month1;
        
    return difference;
  }

  updateDate() {
    const quantum = this.props.quantum;
    let startTimeObj;
    let endTimeObj;
  ...
  updateMonth(e) {
    if (this.state.select === "month") {
      if (Object.keys(this.state.times_available[this.state.selected_year][e.target.name]).length === 1) {
        ...
        const start_day = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[start_month]])[0];
        const start_hour = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[start_month]][start_day])[0];
        const start_min = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[start_month]][start_day][start_hour])[0];
        const start_second = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[start_month]][start_day][start_hour][start_min])[0];
                
        const end_day = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[end_month]])[0];
        const end_hour = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[end_month]][end_day])[0];
        const end_min = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[end_month]][end_day][end_hour])[0];
        const end_second = Object.keys(this.state.times_available[this.state.selected_year][this.state.num_tomonth[end_month]][end_day][end_hour][end_min])[0];
        ...
        const startTimeObj = new moment();
        const endTimeObj = new moment();
        ...
        if (this.state.selecting === "startTime") {
          const difference = this.monthsBetween(startTimeObj.valueOf(), this.state.endTimeObj);
        ...
        const difference = this.monthsBetween(endTimeObj.valueOf(), this.state.startTimeObj);
        ...
        if (difference > 10 || difference < 0) {
          ...
          const endString = endTimeObj.format("YYYY/MM");
          this.setState({
            endTime: endString
          });
        }
  ...
  updateDay(e) {
    ...
    if (this.state.select === "day") {
      ...
      if (Object.keys(this.state.times_available[this.state.selected_year][this.state.selected_month][e.target.name]).length === 1) {
        ...
        const hour = Object.keys(this.state.times_available[this.state.selected_year][this.state.selected_month][e.target.name])[0];
        const min = Object.keys(this.state.times_available[this.state.selected_year][this.state.selected_month][e.target.name][hour])[0];
        const sec = Object.keys(this.state.times_available[this.state.selected_year][this.state.selected_month][e.target.name][hour][min])[0];
        ...
        if (this.state.selecting === "startTime") {
          const difference = this.daysBetween(startTimeObj.valueOf(), this.state.endTimeObj);
          if (difference > 10 || difference < 0) {
            this.startChange(startTimeObj, endTimeObj);
          } else {    
            this.props.localUpdate(this.props.id, moment(startTimeObj), moment(this.state.endTimeObj));
            const startString = startTimeObj.format("YYYY/MM/DD");
          ...
          const difference = this.daysBetween(this.state.startTimeObj, endTimeObj.valueOf());
          if (difference > 10 || difference < 0) {
                        
            this.endChange(startTimeObj, endTimeObj);
          } else {
            const tempStart = moment(this.state.startTimeObj);
            tempStart.tz("GMT");
            this.props.localUpdate(this.props.id, tempStart, endTimeObj);
                        
            const endString = endTimeObj.format("YYYY/MM/DD");
  ...
  updateHour(e) {
    ...
    const min = Object.keys(this.state.times_available[this.state.selected_year][this.state.selected_month][this.state.selected_day][hour])[0];
    const sec = Object.keys(this.state.times_available[this.state.selected_year][this.state.selected_month][this.state.selected_day][hour][min])[0];
  ...
  render() {
    ...
    if (this.state.select == "year" || true) {
      for (const year in this.state.times_available) {
        yearButtons.push(
          <Button
    ...
    if (this.state.select == "month" || true) {
      ...
      for (const idx in this.state.times_available[this.state.selected_year]) {
    ...
    if (this.state.selected_day !== undefined && (this.state.select === "day" || true)) {
      ...
      const days = this.state.times_available[this.state.selected_year][this.state.selected_month];
    ...
    if (this.state.select === "hour") {
      ...
      const hours = this.state.times_available[this.state.selected_year][this.state.selected_month][this.state.selected_day];

      for (const idx in hours) {
        const hour = idx;
        buttons.push(
          <Button
