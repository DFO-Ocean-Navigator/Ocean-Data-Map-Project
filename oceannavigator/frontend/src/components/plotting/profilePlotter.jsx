...
export class ProfilePlotter extends React.PureComponent {
  ...
  componentDidMount() {
    GetTimestampsPromise(this.props.query.dataset, this.props.query.variable[0]).then(
      (timeResult) => {
        this.setState({ "timestamps": timeResult.data })
        GetDepthsPromise(this.props.query.dataset, this.props.query.variable[0]).then(
          (depthResult) => {
    ...
    if (this.state.timestamps) {
      ...
      for (let j = 0; j < this.state.timestamps.length; j++) {
        for (let i = 0; i < this.props.query.variable.length; i++) {
          ...
          const query = "/api/v2.0/plot/profile?query=" + encodeURIComponent(stringify(q)) + "&format=data"
