...
export default class TimePicker extends React.Component {
  ...
  parseDate(value) {
    const test_value = moment.tz(value, "GMT");
    return test_value;
  }
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        for (const date in response) {
          ...
          const m = moment.tz(response[date].value, "GMT");
          
          //Create a truncated string
          const utc_m = m.format("YYYY-MM-DD");
        ...
        this.setState({
          ...
          if (update) {
            const keys = Object.keys(dates_dict);
          ...
          const endhour = time.format("HH[Z]");
          ...
          if (this.props.startDate === null || this.props.startDate === undefined) {
            ...
            if (this.state.dates.includes(startTime.format("YYYY-MM-DD"))) {
              ...
              const starthour = startTime.format("HH[Z]");
              this.setState({
                starthour,
                endhour,
              });
  ...
  rangeUpdate(dates) {
    ...
    const startDate = dates.startDate; //new Date(dates.startDate);
    const endDate = dates.endDate;//new Date(dates.endDate);
    
    // Prev Time Used to Retrieve Hour
    const prevStart = this.props.startDate;
    const prevEnd = this.props.date;
  ...
  singleHourUpdate(key, value) {
    ...
    const int_value = parseInt(value.substring(0, value.length - 1));
    let date;
  ...
  */
  isInValidDay(day) {
    const utc_day = day.format("YYYY-MM-DD");
  ...
  findHours(date) {
    ...
    if (formatted in this.state.dates_dict) {
      ...
      const dates = this.state.dates_dict[formatted];
      
      // Loop through all the timestamps on the requested day
      for (const val in dates) {
        const hour = dates[val].format("HH[Z]");
  ...
  render() {
    ...
    if (this.props.range && this.props.startDate !== null) {
      ...
      if (this.props.quantum === "hour") {
        const hours_available = this.findHours(this.props.date);
