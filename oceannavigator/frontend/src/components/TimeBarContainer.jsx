...
export default class TimeBarContainer extends React.Component {
  ...
  setTime(times) {
    const newTimes = jQuery.extend({}, times);
  ...
  animate() {
    const min = this.findMin(this.state.startTimes);
    const max = this.findMax(this.state.endTimes);
    const quantum = this.findQuantum(this.props.timeSources);
  ...
  animateConcurrent() {
    ...
    for (const dataset in this.state.times) {
      const time = new Date(times[dataset]);
  ...
  animateConsecutive(min, max, quantum) {
    ...
    const times = jQuery.extend({}, this.state.times);
        
    for (const dataset in times) {
  ...
  findQuantum(sources) {
    const quantums = ["month", "day", "hour", "minute"];
    let quantum;
    for (const map in sources) {
      for (const layer in sources[map]) {
        for (const index in sources[map][layer]) {
          for (const dataset in sources[map][layer][index]) {
            for (const variable in sources[map][layer][index][dataset]) {
              if (quantum === undefined) {
  ...
  findMax(times) {
    ...
    for (const dataset in times) {
      const time = moment.tz(times[dataset], "GMT");
      if (largest === undefined) {
        largest = time;
      }
      const time_sec = time.valueOf();
      const largest_sec = largest.valueOf();
  ...
  toggleLayer(e) {
    ...
    for (const idx in this.state.showLayer) {

      if (this.state.showLayer[idx] === e.target.id) {
        const newShowLayer = this.state.showLayer;
  ...
  render() {
    self = this;
    const sources = this.props.timeSources;
    //layers = {'global': ['all']}
    const timeBars = [];
    const quantums = [];
    for (const map in sources) {
      for (const layer in sources[map]) {
        ...
        const new_layer = sources[map][layer];//new Set(this.props.timeSources[layer])
        for (const idx in new_layer) {
          for (const dataset in new_layer[idx]) {
            for (const variable in new_layer[idx][dataset]) {
