...
(async () => {
  ...
  try {
    for (const datasetName of Object.keys(config.point_plot.datasets)) {
      const dataset = config.point_plot.datasets[datasetName];
...
async function selectDatasetAndPerformActions(page, coordinates) {
  ...
  await page.evaluate(() => document.querySelector('[name="radio"][value="point"]').click());
