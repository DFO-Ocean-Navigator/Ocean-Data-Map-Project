...
(async () => {
  ...
  try {
    ...
    // Fetch the div class name where the "Dataset" button is present using its id
    const DatasetDivClassName = await page.evaluate(() => {
      const DatasetButton = document.getElementById('right_map');
      return DatasetButton ? DatasetButton.closest('div').className : 'Dataset in Right Map not found';
    });

    console.log('Div class name where the Right Map Dataset is present:', DatasetDivClassName);
