    ...
    for (const datasetName of Object.keys(config.area_plot.datasets)) {
      ...
      switch (datasetName) {
        case 'GIOPS 10 day Forecast 3D - LatLon':
          ...
          await selectDatasetAndPerformActions(page, coordinates);
          break;
...
async function selectDatasetAndPerformActions(page, coordinates) {
    ...
    console.log('Checked the "Compare Datasets" checkbox.');

    await new Promise(r => setTimeout(r, 5000))

    // Fetch the div class name and ID where the "Dataset" button is present using its id
    const DatasetDivInfo = await page.evaluate(() => {
      const DatasetButton = document.getElementById('right_map');
      if (DatasetButton) {
          const div = DatasetButton.closest('div');
          return {
              className: div.className,
          ...
          return { className: 'Dataset in Right Map not found', id: null };
      }
    });

    console.log('Div class name where the Right Map Dataset is present:', DatasetDivInfo.className);
    console.log('Div ID where the Right Map Dataset is present:', DatasetDivInfo.id);
    ...
    const clickedButtonName1 = await page.evaluate(() => {
      const giopsButton = document.querySelector('#right_map .dd-option-button');
      if (giopsButton) {
          giopsButton.click();
          return giopsButton.textContent.trim(); // Return the name of the clicked button
      }
    ...
    const clickedButtonName3 = await page.evaluate(() => {
      const riopsAccordion = document.getElementById('accordion_RIOPS Forecast');
      if (riopsAccordion) {
          const ccgRiopsButton = riopsAccordion.querySelector('.dd-option-button');
          if (ccgRiopsButton) {
              ccgRiopsButton.click();
              return ccgRiopsButton.textContent.trim(); // Return the name of the clicked button
          }
      }
    });
    ...
    // Click on the "Go" button
    const clickedButtonName4 = await page.evaluate(() => {
      const goButton = document.querySelector('#right_map .go-button');
      if (goButton) {
          goButton.click();
          return goButton.textContent.trim(); // Return the name of the clicked button
      }
    ...
    console.log('Clicked button:', clickedButtonName4);

    // Fetch the closest div class name and id where the "Go" button is present
    const closestDivInfo = await page.evaluate(() => {
      const goButton = document.querySelector('#right_map .go-button');
      if (goButton) {
          const closestDiv = goButton.closest('div');
          return {
              className: closestDiv.className,
              id: closestDiv.id
          };
      }
    });

    console.log('Closest div class name:', closestDivInfo.className);
    console.log('Closest div id:', closestDivInfo.id);
