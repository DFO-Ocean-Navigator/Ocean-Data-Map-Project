...
class TestObservationalQueries(unittest.TestCase):
    ...
    def test_get_platform_track(self):
        ...
        results = q.get_platform_track(
            db.session(), p, quantum="hour",
            starttime=datetime.datetime(2018, 2, 6, 14, 31, 36,
    ...
    def test_get_stations_radius(self):
        result = q.get_stations_radius(
            self.session, 34.6249516667, 25.321855, 0.1
        )
        self.assertEqual(len(result), 1)
        result = q.get_stations_radius(
            self.session, 34.6249516667, 25.321855, 1
        )
        self.assertEqual(len(result), 2)
