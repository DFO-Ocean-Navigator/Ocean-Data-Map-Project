...
class TestOceanNavigatorCfg(unittest.TestCase):
    ...
    def test_config_values(self):
        ...
        self.assertEqual(
            self.config.get("bathymetry_file"), '"/data/misc/ETOPO1_Bed_g_gmt4.grd"'
        )
        self.assertEqual(self.config.get("overlay_kml_dir"), '"./kml"')
        self.assertEqual(
            self.config.get("drifter_agg_url"),
            '"http://localhost:8080/thredds/dodsC/drifter/aggregated.ncml"',
        )
        self.assertEqual(
            self.config.get("drifter_url"),
            '"http://localhost:8080/thredds/dodsC/drifter/%s.nc"',
        )
        self.assertEqual(
            self.config.get("drifter_catalog_url"),
            '"http://localhost:8080/thredds/catalog/drifter/catalog.xml"',
        )
        self.assertEqual(
            self.config.get("observation_agg_url"),
            '"http://localhost:8080/thredds/dodsC/misc/observations/aggregated.ncml"',
        )
        self.assertEqual(
            self.config.get("class4_catalog_url"),
            '"http://localhost:8080/thredds/catalog/class4/catalog.xml"',
        )
        self.assertEqual(
            self.config.get("CLASS4_FNAME_PATTERN"),
            '"http://localhost:8080/thredds/dodsC/class4/%s.nc"',
        )
        self.assertEqual(
            self.config.get("observation_agg_url"),
            '"http://localhost:8080/thredds/dodsC/misc/observations/aggregated.ncml"',
        )
        self.assertEqual(self.config.get("etopo_file"), '"/data/misc/etopo_%s_z%d.nc"')
        self.assertEqual(self.config.get("shape_file_dir"), '"/data/misc/shapes"')
